...
export interface StartServerOptions {
    plugins?: readonly ViralFarmPlugin[];
...
export async function defaultPlugins(): Promise<ViralFarmPlugin[]> {
...
export async function startServer(options: StartServerOptions = {}) {
    ...
    const authProvider = options.authProvider === undefined
        ? await loadAuthProvider(process.env.VIRAL_FARM_AUTH_PLUGIN)
    ...
    console.log(`Viral Farm listening on http://${host}:${address.port}`);
    return {
        app, plugins,
