...
import type { ViralFarmPlugin, TaskDefinition, TaskExecutionContext } from './plugin.js';
...
export function createTikTokPlugin(configuration: TikTokPluginConfiguration = {}): ViralFarmPlugin {
    return {
        id: 'com.viral-farm.tiktok',
        version: '0.1.0',
        ...
        async registerRoutes(context) {
            ...
            context.app.patch<{ Params: { udid: string }; Body: { accounts?: string[] } }>('/api/devices/:udid/accounts', async (request, reply) => {
                ...
                const found = await context.mutateDevices((devices) => {
                    ...
                    device.pluginData = { ...device.pluginData, 'com.viral-farm.tiktok': { ...device.pluginData['com.viral-farm.tiktok'], accounts } };
                    return true;
                });
                ...
                '/api/devices/:udid/fragments/scroll-run', async (request, reply) => {
                    ...
                    try {
                        await context.scheduler.createTask({
                            ...
                            task: {
                                pluginId: 'com.viral-farm.tiktok', taskType: 'doomscroll', taskVersion: 1,
                                payload: {
                        ...
                        }, device.pluginData['com.viral-farm.tiktok'] ?? {});
            ...
            context.app.get<{ Params: { udid: string } }>('/api/devices/:udid/posts/current', async (request) => {
                const latest = (await context.scheduler.listExecutions(25, request.params.udid))
                    .find(({ pluginId, taskType }) => pluginId === 'com.viral-farm.tiktok' && taskType === 'post');
            ...
            context.app.post<{ Params: { udid: string } }>('/api/devices/:udid/posts', async (request, reply) => {
                ...
                try {
                    ...
                    const schedule = await context.scheduler.createTask({
                        deviceUdid: device.udid,
                        task: {
                            pluginId: 'com.viral-farm.tiktok', taskType: 'post', taskVersion: 1,
                            payload: {
                    ...
                    }, device.pluginData['com.viral-farm.tiktok'] ?? {}, new Date(), assetIds);
