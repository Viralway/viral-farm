import type { ViralFarmPlugin, TaskDefinition } from './plugin.js';
import type { CreateTaskInput, JsonObject, TaskEnvelope } from './types.js';

export class PluginRegistry {
    private readonly plugins = new Map<string, ViralFarmPlugin>();
    private readonly tasks = new Map<string, TaskDefinition>();

    constructor(plugins: readonly ViralFarmPlugin[] = []) {
        for (const plugin of plugins) this.register(plugin);
    }

    register(plugin: ViralFarmPlugin): void {
    ...
    list(): ViralFarmPlugin[] {
        return [...this.plugins.values()];
    }

    plugin(id: string): ViralFarmPlugin {
